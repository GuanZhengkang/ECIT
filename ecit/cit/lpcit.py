# LPCIT: https://arxiv.org/pdf/2110.14868

"""
This script is modified from https://github.com/meyerscetbon/lp-ci-test.

Modifications:
+ Added function `lpcit` to simplify the usage of ECIT.

⚠️ Warning: To reproduce the original results, you may need to manually modify scikit-learn's source code,
as required by the original authors. Specifically, in `sklearn/gaussian_process/_gpr.py`,
the `_constrained_optimization` function should be updated to include the following optimizer options:

    options = {
        'disp': None, 'maxcor': 10, 'ftol': 1e-3, 'gtol': 1e-05,
        'eps': 1e-08, 'maxfun': 15000, 'maxiter': 15, 'iprint': -1, 'maxls': 20
    }
"""
...
def lpcit(x,y,z):
    """
    Conducts LPCIT.
    See https://github.com/meyerscetbon/lp-ci-test
    Args:
    ...
    if z.size == 0: # empty conditioning set
        raise ValueError("The conditioning set Z is empty, LPCIT not supported.")
    return test_asymptotic_ci(x,y,z,rank=1000,alpha=0.05)['pvalue']
